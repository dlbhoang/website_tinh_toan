...
export default function TopNavbar({ activePage, setActivePage }) {
  ...
  useEffect(() => {
    if (open && itemRefs.current[open]) {
      const rect = itemRefs.current[open].getBoundingClientRect()
        ...
        <ul className={`tn-list ${mobileOpen ? 'open' : ''}`}>
          {topNavItems.map(item => (
            <li 
              ...
              className={`tn-item${open===item.id?' is-open':''}${isActive(item)?' is-active':''}`}
            >
              ...
              {item.children?.length > 0 && open === item.id && (
                <div className="tn-drop" style={{ left: `${dropdownPos.left}px` }}>
